...
START_TIME = dt.datetime.now().replace(
    year=2024, month=3, day=1, hour=0, minute=0, second=0, microsecond=0
)
...
class Phase:
    ...
    def get_last_n_high(self):
        ...
        if len(self.pm.ticks) < 2:
            raise Exception(
                f"insufficient length to calculate last n high len: {len(self.pm.ticks)}"
            )
        if self.direction == Direction.UP:
            return min(
                self.pm.ticks[
                    -2 - self.pm.settings.LAST_TICKS_BREACH[self.direction] : -2
                ],
    ...
    def on_hard_retracel(self, phase, tick):
        if phase.direction == Direction.UP:
            if self.current_order is not None:
                ...
                self.current_order = None
            logger.info(
                f"hard retracel {phase}, Up change %: {phase.hard_retraced_at.last_price - phase.started_at.last_price}"
            )
        else:
            logger.info(
                f"hard retracel {phase}, Down change %: {phase.started_at.last_price - phase.hard_retraced_at.last_price}"
            )
